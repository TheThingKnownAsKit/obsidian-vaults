...
![[Pasted image 20231205154721.png]]
...
5. To prevent resets, application software kicks the watchdog (resets the timeout count)
...
**Sync Busy**
Note: When executing an operation that requires synchronization, the Synchronization Busy bit in the Status register (`STATUS.bit.SYNCBUSY`) will be set immediately, and cleared when synchronization is complete. Use this line to wait for synchronization:
...
Note: For now, you will be using a clock generator 2 as your source clock. To determine the frequency, you will need to use the equation below to determine the divisor to put into the configuration.
$frequency(Hz)=\frac{32000}{2^{DIV+1}}$

How to create a source clock and assign it to the WDT:
```C++
...
Configure the WDT:
```C++
WDT->CONFIG.bit.PER = period;       // Set period for chip reset from the datasheet WDT->INTENCLR.bit.EW = 1; // Disable early warning interrupt
WDT->CTRL.bit.WEN = 0;                  // Disable window mode
```
Note: You will need to disable the WDT before this code and enable it afterwards.
...
### Task 3 (10 points)
* Write a function that generates the WDT period by arbitrary input
	* Create a Clock (clock generator 2) with a frequency of 2048Hz
...
```
This function can be utilized using: `WDT->CONFIG.bit.PER = setWatchdog(period);`

Test this function in the scenario of Task 1.
...
* Describe the potential importance of knowing the reset cause
